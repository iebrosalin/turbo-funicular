 ...
 */
function initScanAutocomplete(inputElement, scanType) {
    if (!inputElement || !SCAN_KNOWLEDGE_BASE[scanType]) return;
    ...
    window.addEventListener('resize', () => {
        if (suggestionBox && activeInput === inputElement) {
...
function handleInput(e) {
    ...
    const context = detectContext(value, scanType);
    
    // Получаем подсказки на основе контекста
    const suggestions = getSuggestions(context, scanType);
 ...
 */
function detectContext(value, scanType) {
 ...
 */
function getSuggestions(context, scanType) {
    const base = SCAN_KNOWLEDGE_BASE[scanType];
    if (!base) return [];
...
function showSuggestions(suggestions, inputElement) {
    ...
    suggestions.forEach((item, index) => {
        ...
        div.style.cssText = `
            ...
            align-items: center;
        `;
        div.onmouseover = () => selectItem(index);
        div.onclick = () => applySuggestion(index);
...
function applySuggestion(index) {
    if (index < 0 || index >= currentSuggestions.length || !activeInput) return;
...
function handleKeydown(e) {
    ...
    const itemsCount = currentSuggestions.length;
