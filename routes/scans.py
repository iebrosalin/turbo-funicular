...
def start_nmap():
    ...
    if custom_args: scan_job.error_message = f'Custom args: {custom_args}'
    db.session.add(scan_job)
    db.session.commit()
...
def get_scan_results(job_id):
    scan_job = ScanJob.query.get_or_404(job_id)
    results = [{'ip': r.ip_address, 'ports': __import__('json').loads(r.ports) if r.ports else [], 'services': __import__('json').loads(r.services) if r.services else [], 'os': r.os_detection, 'scanned_at': r.scanned_at.strftime('%Y-%m-%d %H:%M:%S')} for r in scan_job.results]
