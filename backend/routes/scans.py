from fastapi import APIRouter, Depends, HTTPException, status, BackgroundTasks, UploadFile, File, Body, Request, Form, Query
...
async def get_scan_jobs(db: AsyncSession = Depends(get_db)):
    ...
    return [{
        ...
        "scan_id": job.scan_id,
        "job_type": job.job_type,
        "scan_type": job.job_type,  # Алиас для совместимости с фронтендом
        "status": job.status,
        "target": job.scan.target if job.scan else "Unknown",
        "progress": getattr(job.scan, 'progress', 0) if job.scan else 0,
        "created_at": job.created_at.isoformat() if job.created_at else None,
...
async def download_scan_job_result(job_id: int, format: str, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logger.exception(f"[DOWNLOAD] Ошибка при скачивании результатов: {e}")
        raise
